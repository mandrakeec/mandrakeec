...
def reiniciar_apache():
    """
    Reinicia Apache en el servidor remoto.
    """
    ip = ip_entry.get()
    user = user_entry.get()
    password = password_entry.get()
    ...
    if sudo_password:
        ...
        execute_ssh_command(ip, user, password, commands,
                            sudo_password=sudo_password)


def verificar_espacio_disco():
    """
    Verifica el espacio en disco del servidor remoto.
    """
    ip = ip_entry.get()
    user = user_entry.get()
    password = password_entry.get()
...
def listar_archivos_logs():
    ...
    commands = [
        'ls -lah /var/www/wstpg_notifications_fcm/storage/logs/'
    ]
    execute_ssh_command(ip, user, password, commands)
...
ttk.Button(root, text='Verificar Espacio en Disco', command=verificar_espacio_disco).grid(
    row=3, column=0, columnspan=2, padx=20, pady=10, sticky='ew')

ttk.Button(root, text='Listar Archivos Logs', command=listar_archivos_logs).grid(
    row=4, column=0, columnspan=2, padx=20, pady=10, sticky='ew')

ttk.Button(root, text='Borrar Logs', command=borrar_logs).grid(
    row=5, column=0, columnspan=2, padx=20, pady=10, sticky='ew')

ttk.Button(root, text='Reiniciar Apache', command=reiniciar_apache).grid(
